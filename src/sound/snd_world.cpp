											   ...
											   int current44kHz, int numSpeakers, float *finalMixBuffer)
{
	...
	// global volume scale
	volume *= soundSystemLocal.dB2Scale(scaleVolumeToDb(idSoundSystemLocal::s_volume.GetFloat()));
...
/*
=================
idSoundWorldLocal::scaleVolumeToDb

Used to scale the volume to dB from the Cvar command.
=================
*/
float idSoundWorldLocal::scaleVolumeToDb(float volume)
{
	if (volume < 0.0)
	{
		// Mute sound :)
	...
	{
		// Scales the volume using linear transformation.
		const float scaledVolume = (volume * 120.0) - 60.0;
		return scaledVolume;
	}
