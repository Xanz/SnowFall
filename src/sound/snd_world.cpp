...
idCVar s_volume("volume", "0", CVAR_ARCHIVE | CVAR_FLOAT, "game volume value 0 - 1", 0, 1);
...
{
public:
	idActiveChannel() : channel(NULL),
						sortKey(0) {}
	idActiveChannel(idSoundChannel *channel_, int sortKey_) : channel(channel_),
															  sortKey(sortKey_) {}
...
idSoundEmitter *idSoundWorldLocal::EmitterForIndex(int index)
{
	...
	{
		common->Printf("idSoundWorldLocal::EmitterForIndex: %i >= %i", index, emitters.Num());
	}
