...
void idSoundHardware_OpenAL::PrintALCInfo(ALCdevice *device)
{
	...
	if (device)
	{
		...
		common->Printf("** Info for device \"%s\" **\n", devname);
	}
	...
	{
		common->Printf("ALC version: %d.%d\n", major, minor);
	}

	if (device)
	{
		common->Printf("OpenAL extensions: %s", alGetString(AL_EXTENSIONS));

		// common->Printf("ALC extensions:");
...
{
	common->Printf("OpenAL vendor string: %s\n", alGetString(AL_VENDOR));
	common->Printf("OpenAL renderer string: %s\n", alGetString(AL_RENDERER));
	common->Printf("OpenAL version string: %s\n", alGetString(AL_VERSION));
	common->Printf("OpenAL extensions: %s", alGetString(AL_EXTENSIONS));
...
{
	common->Printf("Available playback devices:\n");
	...
	// common->Printf("Default capture device: %s\n", alcGetString(NULL, ALC_CAPTURE_DEFAULT_DEVICE_SPECIFIER));
