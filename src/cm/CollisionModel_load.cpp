		...
		for (pref = node->polygons; pref; pref = pref->next)
		{
			...
			for (i = 0; i < p->numEdges; i++)
			{
				...
				if (edge->normal[0] == 0.0f && edge->normal[1] == 0.0f && edge->normal[2] == 0.0f)
				{
					...
					if (edge->numUsers == 1)
					{
						dir = model->vertices[edge->vertexNum[edgeNum < 0]].p - model->vertices[edge->vertexNum[edgeNum
							> 0]].p;
...
cm_polygonRef_t* idCollisionModelManagerLocal::AllocPolygonReference(cm_model_t* model, int blockSize)
{
	...
	if (!model->polygonRefBlocks || !model->polygonRefBlocks->nextRef)
	{
		prefBlock = (cm_polygonRefBlock_t*)Mem_ClearedAlloc(
			sizeof(cm_polygonRefBlock_t) + blockSize * sizeof(cm_polygonRef_t), TAG_COLLISION);
...
cm_brushRef_t* idCollisionModelManagerLocal::AllocBrushReference(cm_model_t* model, int blockSize)
{
	...
	if (!model->brushRefBlocks || !model->brushRefBlocks->nextRef)
	{
		brefBlock = (cm_brushRefBlock_t*)Mem_ClearedAlloc(
			sizeof(cm_brushRefBlock_t) + blockSize * sizeof(cm_brushRef_t), TAG_COLLISION);
...
void idCollisionModelManagerLocal::AddPolygonToNode(cm_model_t* model, cm_node_t* node, cm_polygon_t* p)
{
	...
	pref = AllocPolygonReference(model, model->numPolygonRefs < REFERENCE_BLOCK_SIZE_SMALL
...
void idCollisionModelManagerLocal::AddBrushToNode(cm_model_t* model, cm_node_t* node, cm_brush_t* b)
{
	...
	bref = AllocBrushReference(model, model->numBrushRefs < REFERENCE_BLOCK_SIZE_SMALL
		                                  ? REFERENCE_BLOCK_SIZE_SMALL
		                                  : REFERENCE_BLOCK_SIZE_LARGE);
	bref->b = b;
...
============
*/
int idCollisionModelManagerLocal::R_ChoppedAwayByProcBSP(int nodeNum, idFixedWinding* w, const idVec3& normal,
...
idFixedWinding* idCollisionModelManagerLocal::WindingOutsideBrushes(idFixedWinding* w, const idPlane& plane,
                                                                    int contents, int primitiveNum, cm_node_t* headNode)
{
	int i, windingLeft;
...
=============
*/
void idCollisionModelManagerLocal::ReplacePolygons(cm_model_t* model, cm_node_t* node, cm_polygon_t* p1,
...
=============
*/
bool idCollisionModelManagerLocal::MergePolygonWithTreePolygons(cm_model_t* model, cm_node_t* node,
...
================
*/
void idCollisionModelManagerLocal::R_FilterPolygonIntoTree(cm_model_t* model, cm_node_t* node, cm_polygonRef_t* pref,
                                                           cm_polygon_t* p)
{
	assert(node != NULL);
...
================
*/
void idCollisionModelManagerLocal::R_FilterBrushIntoTree(cm_model_t* model, cm_node_t* node, cm_brushRef_t* pref,
                                                         cm_brush_t* b)
{
	assert(node != NULL);
	while (node->planeType != -1)
...
================
*/
cm_node_t* idCollisionModelManagerLocal::R_CreateAxialBSPTree(cm_model_t* model, cm_node_t* node,
...
================
*/
int idCollisionModelManagerLocal::GetEdge(cm_model_t* model, const idVec3& v1, const idVec3& v2, int* edgeNum,
                                          int v1num)
{
	int v2num, hashKey, e;
	int found, *vertexNum;
...
================
*/
void idCollisionModelManagerLocal::CreatePolygon(cm_model_t* model, idFixedWinding* w, const idPlane& plane,
...
================
*/
void idCollisionModelManagerLocal::PolygonFromWinding(cm_model_t* model, idFixedWinding* w, const idPlane& plane,
                                                      const idMaterial* material, int primitiveNum)
{
	int contents;
...
=================
*/
void idCollisionModelManagerLocal::CreatePatchPolygons(cm_model_t* model, idSurface_Patch& mesh,
                                                       const idMaterial* material, int primitiveNum)
{
	int i, j;
	float dot;
...
cm_model_t* idCollisionModelManagerLocal::LoadBinaryModelFromFile(idFile* file, ID_TIME_T sourceTimeStamp)
{
	...
	struct local
	{
		static void ReadNodeTree(idFile* file, cm_model_t* model, cm_node_t* node, idList<cm_polygon_t*>& polys,
		                         idList<cm_brush_t*>& brushes)
		{
	...
	assert(
		model->brushRefBlocks == NULL || ( model->brushRefBlocks->next == NULL && model->brushRefBlocks->nextRef == NULL
		));
	assert(
		model->polygonRefBlocks == NULL || ( model->polygonRefBlocks->next == NULL && model->polygonRefBlocks->nextRef
			== NULL ));
...
void idCollisionModelManagerLocal::WriteBinaryModelToFile(cm_model_t* model, idFile* file, ID_TIME_T sourceTimeStamp)
{
	...
	struct local
	{
		...
		static void WriteNodeTree(idFile* file, cm_node_t* node, idList<cm_polygon_t*>& polys,
		                          idList<cm_brush_t*>& brushes)
		{
