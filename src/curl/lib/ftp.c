 ...
 */
static CURLcode ftp_getfiletime(struct connectdata *conn, char *file)
{
  CURLcode result = CURLE_OK;
...
static CURLcode ftp_getsize(struct connectdata *conn, char *file,
                            curl_off_t *size)
{
  struct SessionHandle *data = conn->data;
  int ftpcode;
...
static CURLcode ftp_use_port(struct connectdata *conn)
{
  struct SessionHandle *data = conn->data;
...
static CURLcode ftp_use_pasv(struct connectdata *conn,
                             bool *connected)
{
  struct SessionHandle *data = conn->data;
  ssize_t nread;
...
CURLcode Curl_ftp_nextconnect(struct connectdata *conn)
{
  ...
  else if (!data->set.no_body)
  {
    ...
    if ((data->set.ftp_list_only) || !ftp->file)
    {
      ...
      FTPSENDF(conn, "%s",
               data->set.customrequest ? data->set.customrequest : (data->set.ftp_list_only ? "NLST" : "LIST"));
    }
    else
    {
