	...
	if (visible && !noEvents)
	{
		if (event->evType == SE_KEY)
		{
			...
			if (event->evValue == K_MOUSE1)
			{
				...
				while (--c >= 0)
				{
					if (children[c]->visible && children[c]->Contains(children[c]->drawRect, gui->CursorX(),
					                                                  gui->CursorY()) && !(children[c]->noEvents))
					{
						idWindow* child = children[c];
						if (event->evValue2)
						{
			...
			else if (event->evValue == K_MOUSE2)
			{
				...
				while (--c >= 0)
				{
					                                                  ...
					                                                  gui->CursorY()) && !(children[c]->noEvents))
					{
						...
						if (child->Contains(child->clientRect, gui->CursorX(), gui->CursorY()) || GetCaptureChild() ==
							child)
						{
							if ((gui_edit.GetBool() && (child->flags & WIN_SELECTED)) || (!gui_edit.GetBool() && (child
								->flags & WIN_MOVABLE)))
							{
								SetCapture(child);
							}
...
void idWindow::Redraw(float x, float y, bool hud)
{
	...
	if (gui_edit.GetBool() || (flags & WIN_DESKTOP && !(flags & WIN_NOCURSOR) && !hideCursor && (gui->Active() || (flags
		& WIN_MENUGUI))))
	{
