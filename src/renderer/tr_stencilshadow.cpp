										  ...
										  bool makeClippedPlanes)
{
	...
	if (callOptimizer)
	{
		...
		if (numShadowIndexes + opt.totalIndexes > MAX_SHADOW_INDEXES || numShadowVerts + opt.numVerts > MAX_SHADOW_VERTS)
		{
			overflowed = true;
...
void R_MakeShadowFrustums(idRenderLightLocal *light)
{
	...
	if (light->parms.pointLight)
	{
		...
		if (fabs(light->parms.lightCenter[0]) > light->parms.lightRadius[0] || fabs(light->parms.lightCenter[1]) > light->parms.lightRadius[1] || fabs(light->parms.lightCenter[2]) > light->parms.lightRadius[2])
		{
			centerOutside = true;
		}
