...
static void RB_RenderInteraction(const drawSurf_t *surf)
{
	...
	for (int i = 0; i < surfaceShader->GetNumStages(); i++)
	{
		...
		if (surfaceStage->lighting == SL_DIFFUSE || surfaceStage->lighting == SL_SPECULAR)
		{
			...
			// development aid
			if ((surfaceStage->lighting == SL_DIFFUSE && r_skipDiffuse.GetBool()) || (surfaceStage->lighting == SL_SPECULAR && r_skipSpecular.GetBool()))
			{
			...
			for (int j = 0; j < lightShader->GetNumStages(); j++)
			{
				...
				color[0] = backEnd.lightScale * lightRegs[lightStage->color.registers[0]] * surfaceRegs[surfaceStage->color.registers[0]];
				color[1] = backEnd.lightScale * lightRegs[lightStage->color.registers[1]] * surfaceRegs[surfaceStage->color.registers[1]];
				color[2] = backEnd.lightScale * lightRegs[lightStage->color.registers[2]] * surfaceRegs[surfaceStage->color.registers[2]];
				color[3] = 1;
