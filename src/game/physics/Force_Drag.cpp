...
void idForce_Drag::Evaluate(int time)
{
	...
	physics->SetAngularVelocity(rotation.ToAngularVelocity() / MS2SEC(FPS_TO_MSEC()), id);

	velocity = physics->GetLinearVelocity(id) * damping + dir1 * ((l1 - l2) * (1.0f - damping) / MS2SEC(FPS_TO_MSEC()));
