...
void idActor::Spawn()
{
	...
	if (spawnArgs.GetString("sound_bone", "", jointName))
	{
		...
		{
			gameLocal.Warning("idAnimated '%s' at (%s): cannot find joint '%s' for sound playback", name.c_str(),
			                  GetPhysics()->GetOrigin().ToString(0), jointName.c_str());
		}
	}
...
void idActor::Attach(idEntity* ent)
{
	...
	if (joint == INVALID_JOINT)
	{
		gameLocal.Error("Joint '%s' not found for attaching '%s' on '%s'", jointName.c_str(), ent->GetClassname(),
		                name.c_str());
	}
                     ...
                     const char* damageDefName, const float damageScale, const int location)
{
	...
	if (finalBoss && idStr::FindText(GetEntityDefName(), "monster_boss_cyberdemon") == 0 && !inflictor->IsType(
	...
	if (finalBoss && idStr::Icmp(GetEntityDefName(), "monster_hunter_helltime") == 0 && idStr::Icmp(
		inflictor->GetEntityDefName(), "projectile_helltime_killer"))
	{
		return;
	}
	...
	if (!idStr::Icmp(GetEntityDefName(), "monster_boss_d3xp_maledict") &&
		(!idStr::Icmp(damageDefName, "damage_maledict_asteroid") || !idStr::Icmp(
			damageDefName, "damage_maledict_asteroid_splash")))
	{
		return;
	}
		...
		if (health <= 0)
		{
			...
			if (oldHealth > 0)
			{
				...
				if (player != NULL)
				{
					...
					{
						player->GetAchievementManager().
						        EventCompletesAchievement(ACHIEVEMENT_KILL_20_ENEMY_FISTS_HANDS);
					}
					...
					if ((!name.Find("monster_boss_vagary") || !name.Find("vagaryaftercin")) && player->
						GetExpansionType() == GAME_BASE)
					{
					...
					if (inflictor && inflictor->IsType(idMoveable::Type))
					{
						...
						if (moveable != NULL && moveable->GetAttacker() != NULL && moveable->GetAttacker()->
							IsType(idPlayer::Type) && moveable->GetAttacker() == attacker && player->GetExpansionType()
							== GAME_D3XP && team != player->team)
						{
							player->GetAchievementManager().
							        EventCompletesAchievement(ACHIEVEMENT_GRABBER_KILL_20_ENEMY);
						}
					}
					...
					if (inflictor != NULL && inflictor->IsType(idProjectile::Type))
					{
						...
						if (projectile != NULL)
						{
							if (projectile->GetLaunchedFromGrabber() && player->GetExpansionType() == GAME_D3XP && team
								!= player->team)
							{
								player->GetAchievementManager().EventCompletesAchievement(
									ACHIEVEMENT_GRABBER_KILL_20_ENEMY);
							}
							if (renderEntity.hModel && idStr::Icmp(renderEntity.hModel->Name(),
					...
					if (player->health == 1 && player->team != this->team)
					{
						// make sure it doesn't unlock if you kill a friendly dude when you have 1 heath....
						player->GetAchievementManager().EventCompletesAchievement(
