...
void idMenuScreen_HUD::UpdateWeaponInfo(idPlayer* player)
{
	...
	if (ammoAmount == -1 || player->GetCurrentWeaponSlot() == player->weapon_bloodstone || player->
		GetCurrentWeaponSlot() == player->weapon_soulcube)
	{
...
void idMenuScreen_HUD::UpdateCursorState()
{
	...
	else
	{
		if (cursorTalking)
		{
			...
			else if (cursorTalking == 2)
			{
				// already talking / busy
			}
			...
			if (cursorState != CURSOR_TALK)
			{
				...
				if (talkCursor)
				{
					...
					idSWFSpriteInstance* topBacking = talkCursor->GetScriptObject()->GetNestedSprite(
						"backing", "topBar");
					idSWFSpriteInstance* bottomBacking = talkCursor->GetScriptObject()->GetNestedSprite(
						"backing", "botBar");
					...
					idSWFTextInstance* txtPrompt = talkCursor->GetScriptObject()->GetNestedText(
						"talkPrompt", "txtPrompt");
...
void idMenuScreen_HUD::UpdateCommunication(bool show, idPlayer* player)
{
	...
	if (show && !communication->IsVisible())
	{
		...
		{
			idSWFSpriteInstance* node = communication->GetScriptObject()->GetNestedSprite(
...
void idMenuScreen_HUD::UpdateFlashlight(idPlayer* player)
{
	...
	if (player->flashlightBattery != flashlight_batteryDrainTimeMS.GetInteger())
	{
		...
		if (batteryLife)
		{
			float power = ((float)player->flashlightBattery / (float)flashlight_batteryDrainTimeMS.GetInteger()) *
				100.0f;
...
void idMenuScreen_HUD::UpdateChattingHud(idPlayer* player)
{
	...
	else
	{
		if (!mpChatObject->IsVisible())
		{
			...
			idSWFScriptObject* const sayObj = mpChatObject->GetScriptObject()->GetNestedObj(
