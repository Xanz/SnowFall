...
void idGrabber::StartDrag(idEntity* grabEnt, int id)
{
	...
	drag.SetPhysics(phys, clipModelId,
	                thePlayer->firstPersonViewOrigin + localPlayerPoint * thePlayer->firstPersonViewAxis);
	...
	if (dragEnt.IsValid())
	{
		...
		else
		{
			...
			ent->ApplyImpulse(thePlayer, 0, ent->GetPhysics()->GetOrigin(),
			                  thePlayer->firstPersonViewAxis[0] * THROW_SCALE * ent->GetPhysics()->GetMass());
...
int idGrabber::Update(idPlayer* player, bool hide)
{
	...
	if (endTime > gameLocal.time)
	{
		...
		if (!common->IsMultiplayer() && !abort && (player->usercmd.impulseSequence != oldImpulseSequence) && (player->
			usercmd.impulse == IMPULSE_13))
		{
			abort = true;
		}
	...
	if (!dragEnt.GetEntity())
	{
		...
		gameLocal.clip.TraceBounds(trace, player->firstPersonViewOrigin, end, bounds,
		                           MASK_SHOT_RENDERMODEL | CONTENTS_PROJECTILE | CONTENTS_MOVEABLECLIP, player);
...
void idGrabber::UpdateBeams()
{
	...
	if (dragEnt.IsValid())
	{
		...
		{
			thePlayer->weapon.GetEntity()->GetJointWorldTransform(muzzle_joint, gameLocal.time, muzzle_origin,
			                                                      muzzle_axis);
		}
		else
		{
