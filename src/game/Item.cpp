...
void idItem::ReadFromSnapshot(const idBitMsg& msg)
{
	...
	{
		// Fix mispredictions
		if (gameLocal.GetLastClientUsercmdMilliseconds(gameLocal.GetLocalClientNum()) >=
			clientPredictPickupMilliseconds)
		{
...
void idItem::Event_DropToFloor()
{
	...
	gameLocal.clip.TraceBounds(trace, renderEntity.origin, renderEntity.origin - idVec3(0, 0, 64), renderEntity.bounds,
	                           MASK_SOLID | CONTENTS_CORPSE, this);
...
function_t* idItemTeam::LoadScript(const char* script)
{
	...
	if (funcname.Length())
	{
		...
		{
#ifdef _DEBUG
			gameLocal.Warning("idItemTeam '%s' at (%s) calls unknown function '%s'", name.c_str(),
			                  GetPhysics()->GetOrigin().ToString(0), funcname.c_str());
#endif
		}
	}
	return function;
...
void idItemTeam::Think()
{
	...
	if (common->IsServer() && nuggetName && carried && (!lastNuggetDrop || (gameLocal.time - lastNuggetDrop) > spawnArgs
		.GetInt("nugget_frequency")))
	{
	...
	if (dropped && !carried && lastDrop != 0 && (gameLocal.time - lastDrop) > (si_flagDropTimeLimit.GetInteger() *
		1000))
	{
...
{
	idAngles angle(gameLocal.random.RandomInt(spawnArgs.GetInt("nugget_pitch", "30")),
	...
	idEntity* ent = idMoveableItem::DropItem(nuggetName, pos, GetPhysics()->GetAxis(),
				...
				for (int i = 0; i < gameLocal.num_entities; i++)
				{
					if (gameLocal.entities[i] && gameLocal.entities[i]->IsType(idObjectiveComplete::Type))
					{
						if (idStr::Icmp(spawnArgs.GetString("objectivetitle"),
						                gameLocal.entities[i]->spawnArgs.GetString("objectivetitle")) == 0)
						{
...
bool idVideoCDItem::GiveToPlayer(idPlayer* player, unsigned int giveFlags)
{
	...
	const idDeclVideo* video = static_cast<const idDeclVideo*>(declManager->FindType(
		DECL_VIDEO, spawnArgs.GetString("video"), false));
	if (video == NULL)
	{
		return false;
	}
...
void idMoveableItem::Think()
{
	...
	{
		if (!gameLocal.smokeParticles->EmitSmoke(smoke, smokeTime, gameLocal.random.CRandomFloat(),
...
================
*/
idEntity* idMoveableItem::DropItem(const char* classname, const idVec3& origin, const idMat3& axis,
                                   const idVec3& velocity, int activateDelay, int removeDelay)
{
	idDict args;
	idEntity* item;
...
void idMoveableItem::Gib(const idVec3& dir, const char* damageDefName)
{
	...
	if (*smokeName != '\0')
	{
		const idDeclParticle* smoke = static_cast<const idDeclParticle*>(declManager->
			FindType(DECL_PARTICLE, smokeName));
		gameLocal.smokeParticles->EmitSmoke(smoke, gameLocal.time, gameLocal.random.CRandomFloat(), renderEntity.origin,
		                                    renderEntity.axis, timeGroup /*_D3XP*/);
	}
	// remove the entity
	PostEventMS(&EV_Remove, 0);
}
