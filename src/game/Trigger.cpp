...
void idTrigger::DrawDebugInfo()
{
	...
	for (ent = gameLocal.spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next())
	{
		if (ent->GetPhysics()->GetContents() & (CONTENTS_TRIGGER | CONTENTS_FLASHLIGHT_TRIGGER))
		{
			...
			{
				gameRenderWorld->DrawText(ent->name.c_str(), ent->GetPhysics()->GetAbsBounds().GetCenter(), 0.1f,
				                          colorWhite, axis, 1);
				gameRenderWorld->DrawText(ent->GetEntityDefName(), ent->GetPhysics()->GetAbsBounds().GetCenter() + up,
				...
				if (func)
				{
					gameRenderWorld->DrawText(va("call script '%s'", func->Name()),
					                          ent->GetPhysics()->GetAbsBounds().GetCenter() - up, 0.1f, colorWhite,
					                          axis, 1);
				}
			}
			...
			for (i = 0; i < ent->targets.Num(); i++)
			{
				...
				if (target)
				{
					gameRenderWorld->DebugArrow(colorYellow, ent->GetPhysics()->GetAbsBounds().GetCenter(),
					...
					{
						gameRenderWorld->DrawText(target->name.c_str(),
						                          target->GetPhysics()->GetAbsBounds().GetCenter(), 0.1f, colorWhite,
						                          axis, 1);
					}
				}
			}
...
void idTrigger::Restore(idRestoreGame* savefile)
{
	...
	if (funcname.Length())
	{
		...
		{
			gameLocal.Warning("idTrigger_Multi '%s' at (%s) calls unknown function '%s'", name.c_str(),
...
void idTrigger::Spawn()
{
	...
	if (funcname.Length())
	{
		...
		if (scriptFunction == NULL)
		{
			gameLocal.Warning("trigger '%s' at (%s) calls unknown function '%s'", name.c_str(),
			                  GetPhysics()->GetOrigin().ToString(0), funcname.c_str());
		}
	}
	else
...
void idTrigger_Multi::Spawn()
{
	...
	{
		random = wait - 1;
		gameLocal.Warning("idTrigger_Multi '%s' at (%s) has random >= wait", name.c_str(),
	...
	if (random_delay && (random_delay >= delay) && (delay >= 0))
	{
		...
		gameLocal.Warning("idTrigger_Multi '%s' at (%s) has random_delay >= delay", name.c_str(),
		                  GetPhysics()->GetOrigin().ToString(0));
	}
...
void idTrigger_EntityName::Spawn()
{
	...
	{
		random = wait - 1;
		gameLocal.Warning("idTrigger_EntityName '%s' at (%s) has random >= wait", name.c_str(),
		                  GetPhysics()->GetOrigin().ToString(0));
	}
	...
	{
		random_delay = delay - 1;
		gameLocal.Warning("idTrigger_EntityName '%s' at (%s) has random_delay >= delay", name.c_str(),
		                  GetPhysics()->GetOrigin().ToString(0));
	}
...
void idTrigger_Timer::Spawn()
{
	...
	{
		random = wait - 0.001;
		gameLocal.Warning("idTrigger_Timer '%s' at (%s) has random >= wait", name.c_str(),
		                  GetPhysics()->GetOrigin().ToString(0));
	}
...
void idTrigger_Flag::Spawn()
{
	...
	if (funcname.Length())
	{
		...
		// gameLocal.program.FindFunction( funcname );//, &idItemTeam::Type );
		if (eventFlag == NULL)
		{
