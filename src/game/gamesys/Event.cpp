...
idEvent* idEvent::Alloc(const idEventDef* evdef, int numargs, va_list args)
{
	...
	for (i = 0; i < numargs; i++)
	{
		...
		if (format[i] != arg->type)
		{
			...
			if (!(((format[i] == D_EVENT_TRACE) || (format[i] == D_EVENT_ENTITY)) && (arg->type == 'd') && (arg->value
				== 0)))
			{
				gameLocal.Error("idEvent::Alloc : Wrong type passed in for arg # %d on '%s' event.", i,
				                evdef->GetName());
			}
		}
		...
		case D_EVENT_STRING:
			if (arg->value)
			{
				idStr::Copynz(reinterpret_cast<char*>(dataPtr), reinterpret_cast<const char*>(arg->value),
				              MAX_STRING_LEN);
			}
			break;
			...
			if (arg->value)
			{
				...
				*reinterpret_cast<trace_t*>(dataPtr + sizeof(bool)) = *reinterpret_cast<const trace_t*>(arg->value);
				...
				if (reinterpret_cast<const trace_t*>(arg->value)->c.material)
				{
					...
					idStr::Copynz(reinterpret_cast<char*>(dataPtr + sizeof(bool) + sizeof(trace_t)), materialName,
					              MAX_STRING_LEN);
				}
			}
			else
		...
		default:
			gameLocal.Error("idEvent::Alloc : Invalid arg format '%s' string for '%s' event.", format,
			                evdef->GetName());
			break;
		}
	}
...
void idEvent::ServiceEvents()
{
	...
	while (!EventQueue.IsListEmpty())
	{
		...
		for (i = 0; i < numargs; i++)
		{
			...
			case D_EVENT_ENTITY:
			case D_EVENT_ENTITY_NULL:
				*reinterpret_cast<idEntity**>(&args[i]) = reinterpret_cast<idEntityPtr<idEntity>*>(&data[offset])->
					GetEntity();
				break;
...
void idEvent::Restore(idRestoreGame* savefile)
{
	...
	for (i = 0; i < num; i++)
	{
		...
		{
			savefile->Error("idEvent::Restore: unknown class '%s' on event '%s'", name.c_str(),
		...
		{
			savefile->Error("idEvent::Restore: arg size (%d) doesn't match saved arg size(%d) on event '%s'",
