...
============
*/
idVarDef* idProgram::FindFreeResultDef(idTypeDef* type, const char* name, idVarDef* scope, const idVarDef* a,
                                       const idVarDef* b)
{
	idVarDef* def;
...
void idProgram::DisassembleStatement(idFile* file, int instructionPointer) const
{
	...
	file->Printf("%20s(%d):\t%6d: %15s\t", fileList[statement->file].c_str(), statement->linenumber, instructionPointer,
	             op->opname);
...
void idProgram::Disassemble() const
{
	...
	for (i = 0; i < functions.Num(); i++)
	{
		...
		file->Printf("\nfunction %s() %d stack used, %d parms, %d locals {\n", func->Name(), func->locals,
		             func->parmTotal, func->locals - func->parmTotal);
...
bool idProgram::CompileText(const char* source, const char* text, bool console)
{
	...
	try
	{
		...
		{
			def = varDefs[i];
			if ((def->Type() == ev_function) && ((def->scope->Type() == ev_namespace) || def->scope->TypeDef()->
				Inherits(&type_object)))
			{
