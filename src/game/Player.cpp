...
idCVar flashlight_batteryDrainTimeMS("flashlight_batteryDrainTimeMS", "30000", CVAR_INTEGER,
                                     "amount of time (in MS) it takes for full battery to drain (-1 == no battery drain)");
idCVar flashlight_batteryChargeTimeMS("flashlight_batteryChargeTimeMS", "3000", CVAR_INTEGER,
                                      "amount of time (in MS) it takes to fully recharge battery");
idCVar flashlight_minActivatePercent("flashlight_minActivatePercent", ".25", CVAR_FLOAT,
                                     "( 0.0 - 1.0 ) minimum amount of battery (%) needed to turn on flashlight");
idCVar flashlight_batteryFlickerPercent("flashlight_batteryFlickerPercent", ".1", CVAR_FLOAT,
...
void idInventory::RestoreInventory(idPlayer* owner, const idDict& dict)
{
	...
	for (i = 0; i < num; i++)
	{
		...
		videos[i] = static_cast<const idDeclVideo*>(declManager->FindType(
			DECL_VIDEO, dict.GetString(itemname, "default")));
	}
	...
	for (i = 0; i < num; i++)
	{
		...
		emails[i] = static_cast<const idDeclEmail*>(declManager->FindType(
			DECL_EMAIL, dict.GetString(itemname, "default")));
	}
...
==============
*/
void idInventory::AddPickupName(const char* name, idPlayer* owner)
{
	//_D3XP
                       ...
                       idPredictedValue<int>* idealWeapon, bool updateHud, unsigned int giveFlags)
{
	...
	else if (!idStr::Icmp(statname, "weapon"))
	{
		...
		for (pos = value; pos != NULL; pos = end)
		{
			...
			if (common->IsMultiplayer() && (weapons & (1 << i)) && (weaponDecl != NULL) && !weaponDecl->dict.GetInt(
				"ammoRequired"))
			{
				continue;
			}

			if (!gameLocal.world->spawnArgs.GetBool("no_Weapons") || (weaponName == "weapon_fists") || (weaponName ==
				"weapon_soulcube"))
			{
				if ((weapons & (1 << i)) == 0 || common->IsMultiplayer())
				{
					...
					if (giveFlags & ITEM_GIVE_FEEDBACK)
					{
						...
						if (lobby.GetLobbyUserWeaponAutoSwitch(lobbyUserID) && idealWeapon != NULL && i != owner->
							weapon_bloodstone_active1 && i != owner->weapon_bloodstone_active2 && i != owner->
...
void idPlayer::SpawnToPoint(const idVec3& spawn_origin, const idAngles& spawn_angles)
{
	...
	{
		physicsObj.SetClipMask(MASK_PLAYERSOLID);
		// the clip mask is usually maintained in Move(), but KillBox requires it
...
void idPlayer::UpdateSkinSetup()
{
	...
	else
	{
		...
		skinIndex = entityNumber;
		// session->GetActingGameStateLobbyBase().GetLobbyUserSkinIndex( gameLocal.lobbyUserIDs[entityNumber] );
	}
...
void idPlayer::UpdateHudWeapon(bool flashWeapon)
{
	...
	idPlayer* p = this;
	if (gameLocal.GetLocalClientNum() >= 0 && gameLocal.entities[gameLocal.GetLocalClientNum()] && gameLocal.entities[
		gameLocal.GetLocalClientNum()]->IsType(idPlayer::Type))
	{
...
void idPlayer::DrawHUD(idMenuHandler_HUD* _hudManager)
{
	...
	if (!weapon.GetEntity() || influenceActive != INFLUENCE_NONE || privateCameraView || gameLocal.GetCamera() || !
		g_showHud.GetBool())
	{
		return;
	}
...
void idPlayer::FireWeapon()
{
	...
	if (!hiddenWeapon && weapon.GetEntity()->IsReady())
	{
		if (g_infiniteAmmo.GetBool() || weapon.GetEntity()->AmmoInClip() || weapon.GetEntity()->AmmoAvailable())
		{
			...
			if ((weapon_bloodstone >= 0) && (currentWeapon == weapon_bloodstone) && inventory.weapons & (1 <<
				weapon_bloodstone_active1) && weapon.GetEntity()->GetStatus() == WP_READY)
			{
...
void idPlayer::UpdatePowerUps()
{
	...
	if (!gameLocal.inCinematic && influenceActive == 0 && g_skill.GetInteger() == 3 && gameLocal.time > nextHealthTake
...
void idPlayer::GivePDA(const idDeclPDA* pda, const char* securityItem)
{
	...
	{
		pda = static_cast<const idDeclPDA*>(declManager->
			FindType(DECL_PDA, spawnArgs.GetString("pda_name", "personal")));
	}
	if (pda == NULL)
	{
		return;
...
{
	if (!weaponEnabled || spectating || hiddenWeapon || gameLocal.inCinematic || gameLocal.world->spawnArgs.
		GetBool("no_Weapons") || health < 0)
	{
		return;
	}
	...
	if (weaponToggles.Get(va("weapontoggle%d", num), &weaponToggle))
	{
		...
		for (int i = 0; i < weaponToggle->toggleList.Num(); i++)
		{
			...
			if (inventory.weapons & (1 << weapNum))
			{
				...
				if (inventory.HasAmmo(spawnArgs.GetString(va("def_weapon%d", weapNum)), true, this) || spawnArgs.
					GetBool(va("weapon%d_allowempty", weapNum)))
				{
					break;
				}
			}
...
void idPlayer::DropWeapon(bool died)
{
	...
	if (!idStr::Icmp(idWeapon::GetAmmoNameForNum(weapon.GetEntity()->GetAmmoType()), "ammo_grenades") && (ammoavailable
		- inclip <= 0))
	{
		return;
	}
...
void idPlayer::Weapon_GUI()
{
	...
	if (isDown != wasDown)
	{
		...
		{
			if (strcmp(focusGUIent->GetName(), "chamber_gui_console") == 0 && strcmp(
				command, " ; runScript chamber_trigger") == 0)
			{
...
void idPlayer::UpdateFlashlight()
{
	...
	if (!common->IsMultiplayer())
	{
		...
		else
		{
			...
			{
				flashlightBattery += (gameLocal.time - gameLocal.previousTime) * Max(
					1, (flashlight_batteryDrainTimeMS.GetInteger() / flashlight_batteryChargeTimeMS.GetInteger()));
				if (flashlightBattery > flashlight_batteryDrainTimeMS.GetInteger())
				{
	...
	if (flashlight.GetEntity()->lightOn)
	{
		if ((flashlightBattery < flashlight_batteryChargeTimeMS.GetInteger() / 2) && (gameLocal.random.RandomFloat() <
...
void idPlayer::FlashlightOn()
{
	...
	if (flashlightBattery < idMath::Ftoi(
		flashlight_minActivatePercent.GetFloat() * flashlight_batteryDrainTimeMS.GetFloat()))
	{
		return;
	}
...
void idPlayer::UpdateFocus()
{
	...
	for (i = 0; i < listedClipModels; i++)
	{
		...
		if (!ent->GetRenderEntity() || !ent->GetRenderEntity()->gui[0] || !ent->GetRenderEntity()->gui[0]->
			IsInteractive())
		{
			continue;
		}
...
void idPlayer::BobCycle(const idVec3& pushVelocity)
{
	...
	if (!physicsObj.HasGroundContacts() || influenceActive == INFLUENCE_LEVEL2 || (common->IsMultiplayer() &&
		spectating))
	{
		// airborne
		bobCycle = 0;
...
void idPlayer::UpdateViewAngles()
{
	...
	if (!noclip && (gameLocal.inCinematic || privateCameraView || gameLocal.GetCamera() || influenceActive ==
		INFLUENCE_LEVEL2 || objectiveSystemOpen))
	{
	...
	for (i = 0; i < 3; i++)
	{
		...
		{
			viewAngles[i] += idMath::ClampFloat(-1.0f, 1.0f,
			                                    idMath::AngleDelta(
...
{
	int base = idMath::Ftoi(
		(BASE_HEARTRATE + LOWHEALTH_HEARTRATE_ADJ) - ((float)health / 100.0f) * LOWHEALTH_HEARTRATE_ADJ);
...
void idPlayer::PerformImpulse(int impulse)
{
	...
	case IMPULSE_16:
		{
			if (flashlight.IsValid())
			{
				...
				else if (!spectating && weaponEnabled && !hiddenWeapon && !gameLocal.world->spawnArgs.GetBool(
					"no_Weapons"))
				{
					FlashlightOn();
				}
	...
	case IMPULSE_25:
		{
			if (common->IsServer() && gameLocal.mpGame.IsGametypeFlagBased() && (gameLocal.serverInfo.
				GetInt("si_midnight") == 2))
			{
				if (enviroSuitLight.IsValid())
				{
					enviroSuitLight.GetEntity()->PostEventMS(&EV_Remove, 0);
...
void idPlayer::AdjustSpeed()
{
	...
	else
	{
		...
		if ((usercmd.forwardmove == 0) && (usercmd.rightmove == 0) && (!physicsObj.OnLadder() || ((usercmd.buttons & (
			BUTTON_CROUCH | BUTTON_JUMP)) == 0)))
		{
			rate *= 1.25f;
		}
	...
	else if ((usercmd.rightmove != 0) && physicsObj.IsCrouching())
	{
		if (!legsForward)
		{
			idealLegsYaw = idMath::AngleNormalize180(
				idVec3(idMath::Abs(usercmd.rightmove), usercmd.rightmove, 0.0f).ToYaw());
		}
		else
		{
			idealLegsYaw = idMath::AngleNormalize180(
...
void idPlayer::Move_Interpolated(float fraction)
{
	...
	else if (gameLocal.inCinematic || gameLocal.GetCamera() || privateCameraView || (influenceActive ==
		INFLUENCE_LEVEL2))
	{
		physicsObj.SetContents(CONTENTS_BODY);
...
void idPlayer::Move()
{
	...
	else
	{
		...
		if (groundEnt != NULL && groundEnt->IsType(idAI::Type))
		{
			...
			{
				vel.ToVec2() = physicsObj.GetOrigin().ToVec2() - groundEnt->GetPhysics()->GetAbsBounds().GetCenter().
				                                                            ToVec2();
...
bool idPlayer::AllowClientAuthPhysics()
{
	...
	return (usercmd.serverGameMilliseconds > serverOverridePositionTime && commonLocal.GetUCmdMgr().
		HasUserCmdForPlayer(entityNumber));
}
...
void idPlayer::RunPhysics_RemoteClientCorrection()
{
	if (!AllowClientAuthPhysics())
	{
		...
		if (pm_clientAuthoritative_debug.GetBool())
		{
			...
			idLib::Printf("[%d]Ignoring client auth:  cmd.serverTime: %d  overrideTime: %d \n", entityNumber,
			              usercmd.serverGameMilliseconds, serverOverridePositionTime);
		}
		return;
	}
	...
	// Correction
	{
		...
		if (delta > pm_clientAuthoritative_minDist.GetFloat())
		{
			...
			{
				desiredPos.x = idMath::LerpToWithScale(newOrigin.x, desiredPos.x,
				                                       pm_clientAuthoritative_Lerp.GetFloat());
				desiredPos.y = idMath::LerpToWithScale(newOrigin.y, desiredPos.y,
				                                       pm_clientAuthoritative_Lerp.GetFloat());
			}
...
void idPlayer::ShowRespawnHudMessage()
{
	...
	else
	{
		...
		outMsg.BeginWriting();
		outMsg.WriteByte(0);
		session->GetActingGameStateLobbyBase().SendReliableToLobbyUser(gameLocal.lobbyUserIDs[entityNumber],
	...
	if (gameLocal.realClientTime == lastMPAimTime)
	{
			...
			&& static_cast<idPlayer*>(gameLocal.entities[MPAim])->team == team)
		{
			...
			hud->TriggerHitTarget(
				true, session->GetActingGameStateLobbyBase().GetLobbyUserName(gameLocal.lobbyUserIDs[MPAim]),
				aimed->team + 1);
			MPAimHighlight = true;
...
void idPlayer::Think()
{
	...
	if (g_showEnemies.GetBool())
	{
		...
		for (ent = enemyList.Next(); ent != NULL; ent = ent->enemyNode.Next())
		{
			...
			gameRenderWorld->DebugBounds(colorRed, ent->GetPhysics()->GetBounds().Expand(2),
	...
	gameLocal.portalSkyActive = gameLocal.pvs.CheckAreasForPortalSky(gameLocal.GetPlayerPVS(),
	...
	if (common->IsMultiplayer() && session->GetState() >= idSession::INGAME && entityNumber < MAX_CLIENTS)
	{
...
============
*/
void idPlayer::ServerDealDamage(int damage, idEntity& inflictor, idEntity& attacker, const idVec3& dir,
                                const char* damageDefName, const int location)
{
	...
	if (damage > 0)
	{
		...
		if (health <= 0)
		{
			...
			if (oldHealth > 0 && strcmp(gameLocal.GetMapName(), "maps/game/mp/d3dm3.map") == 0 && strcmp(
				damageDefName, "damage_triggerhurt_1000_chamber") == 0)
			{
				idPlayer* fragChamberActivator = gameLocal.playerActivateFragChamber;
				if (fragChamberActivator != NULL)
				{
					fragChamberActivator->GetAchievementManager().EventCompletesAchievement(
						ACHIEVEMENT_MP_CATCH_ENEMY_IN_ROFC);
				}
                      ...
                      const char* damageDefName, const float damageScale, const int location)
{
	...
	if (g_debugDamage.GetInteger())
	{
		gameLocal.Printf("client:%02d\tdamage type:%s\t\thealth:%03d\tdamage:%03d\tarmor:%03d\n", entityNumber,
		                 damageDef->GetName(), health, damage, armorSave);
	}
...
idAngles idPlayer::GunTurningOffset()
{
	...
	weapon.GetEntity()->GetWeaponAngleOffsets(&weaponAngleOffsetAverages, &weaponAngleOffsetScale,
...
void idPlayer::CalculateFirstPersonView()
{
	if ((pm_modelView.GetInteger() == 1) || ((pm_modelView.GetInteger() == 2) && (health <= 0)))
	{
		...
		firstPersonViewOrigin = (origin + modelOffset) * (viewAxis * physicsObj.GetGravityAxis()) + physicsObj.
			GetOrigin() + viewBob;
...
void idPlayer::CalculateRenderView()
{
	...
	else
	{
		...
		{
			OffsetThirdPersonView(pm_thirdPersonAngle.GetFloat(), pm_thirdPersonRange.GetFloat(),
		...
		else if (pm_thirdPersonDeath.GetBool())
		{
			range = gameLocal.time < minRespawnTime
				        ? (gameLocal.time + RAGDOLL_DEATH_TIME - minRespawnTime) * (120.0f / RAGDOLL_DEATH_TIME)
				        : 120.0f;
...
void idPlayer::SetLastHitTime(int time)
{
	...
	if (MPAim != -1)
	{
		...
		if (hud)
		{
			...
			hud->TriggerHitTarget(
				true, session->GetActingGameStateLobbyBase().GetLobbyUserName(gameLocal.lobbyUserIDs[MPAim]), color);
		}
		MPAimHighlight = true;
		MPAimFadeTime = 0;
	}
	else if (lastMPAim != -1)
	{
		...
		if (hud)
		{
			...
			hud->TriggerHitTarget(
				true, session->GetActingGameStateLobbyBase().GetLobbyUserName(gameLocal.lobbyUserIDs[lastMPAim]),
				color);
			hud->TriggerHitTarget(false, "");
		}
		MPAimHighlight = false;
...
void idPlayer::Event_ExitTeleporter()
{
	...
	if (teleportKiller != -1)
	{
		...
		Damage(gameLocal.entities[teleportKiller], gameLocal.entities[teleportKiller], vec3_origin, "damage_telefrag",
		       1.0f, INVALID_JOINT);
		teleportKiller = -1;
	}
	else
...
void idPlayer::ReadFromSnapshot(const idBitMsg& msg)
{
	...
	*/
	...
	else if (health < oldHealth && health > 0)
	{
		...
		{
			// damage feedback
			const idDeclEntityDef* def = static_cast<const idDeclEntityDef*>(declManager->DeclByIndex(
				DECL_ENTITYDEF, lastDamageDef, false));
			if (def)
			{
				if (IsLocallyControlled())
				{
...
bool idPlayer::NeedsIcon()
{
	...
	return !IsLocallyControlled() && ((g_CTFArrows.GetBool() && gameLocal.mpGame.IsGametypeFlagBased() && !IsHidden() &&
		!AI_DEAD) || (isLagged));
}
...
void idPlayer::SetControllerShake(float highMagnitude, int highDuration, float lowMagnitude, int lowDuration)
{
	...
	for (int i = 0; i < MAX_SHAKE_BUFFER; i++)
	{
		...
		{
			if (idMath::Fabs(highMagnitude - controllerShakeHighMag[i]) <= 0.1f && idMath::Fabs(
				lowMagnitude - controllerShakeLowMag[i]) <= 0.1f)
			{
