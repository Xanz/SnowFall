...
// Used to calculate the maximum amount of airTics.
int GetMaxAirTic()
{
	return pm_airTics.GetInteger();
}
...
void idPlayer::BobCycle(const idVec3 &pushVelocity)
{
	...
	else
	{
		...
		old = bobCycle;
		bobCycle = (int)(old + bobmove * FPS_TO_MSEC()) & 255;
...
void idPlayer::UpdateAir(void)
{
	...
	if (hud)
	{
		hud->SetStateInt("player_air", 100 * airTics / GetMaxAirTic());
	}
}
...
void idPlayer::AdjustSpeed(void)
{
	...
	{
		// Xanz this fixes no clip speed.
		if (com_engineHz.GetFloat() <= 60)
		{
			speed = pm_noclipspeed.GetFloat();
		}
		else if (com_engineHz.GetFloat() <= 144)
		{
			speed = pm_noclipspeed.GetFloat() / 60 * com_engineHz.GetFloat();
		}
		else
		{
			speed = (pm_noclipspeed.GetFloat() / 60 * com_engineHz.GetFloat()) / 2;
		}
	...
	else
	{
		...
		stamina += rate * MS2SEC(FPS_TO_MSEC());
...
void idPlayer::UpdateHud(void)
{
	...
	{
		if (MPAim != -1 && gameLocal.gameType == GAME_TDM && gameLocal.entities[MPAim] && gameLocal.entities[MPAim]->IsType(idPlayer::Type) && static_cast<idPlayer *>(gameLocal.entities[MPAim])->team == team)
		{
								...
								const float damageScale, const int location, int *health, int *armor)
{
	...
	if (gameLocal.gameType == GAME_TDM && !gameLocal.serverInfo.GetBool("si_teamDamage") && !damageDef->GetBool("noTeam") && player && player != this // you get self damage no matter what
		&& player->team == team)
	{
		damage = 0;
	}
