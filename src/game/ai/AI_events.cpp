...
void idAI::Event_MeleeAttackToJoint(const char *jointname, const char *meleeDefName)
{
	...
	{
		gameRenderWorld->DebugLine(colorYellow, start, end, FPS_TO_MSEC());
	}
...
void idAI::Event_TestChargeAttack(void)
{
	...
	if (ai_debugMove.GetBool())
	{
		gameRenderWorld->DebugLine(colorGreen, physicsObj.GetOrigin(), end, FPS_TO_MSEC());
		gameRenderWorld->DebugBounds(path.endEvent == 0 ? colorYellow : colorRed, physicsObj.GetBounds(), end, FPS_TO_MSEC());
	}
...
void idAI::Event_TestAnimMoveTowardEnemy(const char *animname)
{
	...
	if (ai_debugMove.GetBool())
	{
		gameRenderWorld->DebugLine(colorGreen, physicsObj.GetOrigin(), physicsObj.GetOrigin() + moveVec, FPS_TO_MSEC());
		gameRenderWorld->DebugBounds(path.endEvent == 0 ? colorYellow : colorRed, physicsObj.GetBounds(), physicsObj.GetOrigin() + moveVec, FPS_TO_MSEC());
	}
...
void idAI::Event_TestMoveToPosition(const idVec3 &position)
{
	...
	if (ai_debugMove.GetBool())
	{
		gameRenderWorld->DebugLine(colorGreen, physicsObj.GetOrigin(), position, FPS_TO_MSEC());
		gameRenderWorld->DebugBounds(colorYellow, physicsObj.GetBounds(), position, FPS_TO_MSEC());
		if (path.endEvent)
		{
			gameRenderWorld->DebugBounds(colorRed, physicsObj.GetBounds(), path.endPos, FPS_TO_MSEC());
		}
	}
