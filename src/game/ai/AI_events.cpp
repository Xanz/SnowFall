...
void idAI::Event_CanBecomeSolid()
{
	...
	num = gameLocal.clip.ClipModelsTouchingBounds(physicsObj.GetAbsBounds(), MASK_MONSTERSOLID, clipModels,
	                                              MAX_GENTITIES);
	for (i = 0; i < num; i++)
	{
		cm = clipModels[i];
		...
		if (spawnClearMoveables && hit->IsType(idMoveable::Type) || hit->IsType(idBarrel::Type) || hit->IsType(
...
void idAI::Event_GetJumpVelocity(const idVec3& pos, float speed, float max_height)
{
	...
	result = PredictTrajectory(start, end, speed, physicsObj.GetGravity(), physicsObj.GetClipModel(), MASK_MONSTERSOLID,
	                           max_height, this, enemyEnt, ai_debugMove.GetBool() ? 4000 : 0, dir);
	if (result)
	{
...
void idAI::Event_PredictEnemyPos(float time)
{
	...
	idAI::PredictPath(enemyEnt, aas, lastVisibleEnemyPos, enemyEnt->GetPhysics()->GetLinearVelocity(), SEC2MS(time),
	                  SEC2MS(time), (move.moveType == MOVETYPE_FLY) ? SE_BLOCKED : (SE_BLOCKED | SE_ENTER_LEDGE_AREA),
	                  path);
...
void idAI::Event_TestChargeAttack()
{
	...
	idAI::PredictPath(this, aas, physicsObj.GetOrigin(), end - physicsObj.GetOrigin(), 1000, 1000,
...
void idAI::Event_TestAnimMoveTowardEnemy(const char* animname)
{
	...
	idAI::PredictPath(this, aas, physicsObj.GetOrigin(), moveVec, 1000, 1000,
	                  (move.moveType == MOVETYPE_FLY)
		                  ? SE_BLOCKED
		                  : (SE_ENTER_OBSTACLE | SE_BLOCKED | SE_ENTER_LEDGE_AREA), path);
	...
	if (ai_debugMove.GetBool())
	{
		...
		gameRenderWorld->DebugBounds(path.endEvent == 0 ? colorYellow : colorRed, physicsObj.GetBounds(),
...
void idAI::Event_TestAnimMove(const char* animname)
{
	...
	moveVec = animator.TotalMovementDelta(anim) * idAngles(0.0f, ideal_yaw, 0.0f).ToMat3() * physicsObj.
		GetGravityAxis();
	idAI::PredictPath(this, aas, physicsObj.GetOrigin(), moveVec, 1000, 1000,
	                  (move.moveType == MOVETYPE_FLY)
		                  ? SE_BLOCKED
...
void idAI::Event_TestMoveToPosition(const idVec3& position)
{
	...
	idAI::PredictPath(this, aas, physicsObj.GetOrigin(), position - physicsObj.GetOrigin(), 1000, 1000,
	                  (move.moveType == MOVETYPE_FLY)
		                  ? SE_BLOCKED
		                  : (SE_ENTER_OBSTACLE | SE_BLOCKED | SE_ENTER_LEDGE_AREA), path);
...
void idAI::Event_TestAnimAttack(const char* animname)
{
	...
	idAI::PredictPath(this, aas, physicsObj.GetOrigin(), animator.TotalMovementDelta(anim), 1000, 1000,
	                  (move.moveType == MOVETYPE_FLY)
		                  ? SE_BLOCKED
		                  : (SE_ENTER_OBSTACLE | SE_BLOCKED | SE_ENTER_LEDGE_AREA), path);
...
void idAI::Event_SetSmokeVisibility(int num, int on)
{
	...
	{
		gameLocal.Warning("Particle #%d out of range (%d particles) on entity '%s'", num, particles.Num(),
		                  name.c_str());
		return;
	}
