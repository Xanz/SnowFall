...
void idAASLocal::CalculateAreaTravelTimes()
{
	...
	assert(
		( (ptrdiff_t) bytePtr - (ptrdiff_t) areaTravelTimes ) <= (ptrdiff_t)numAreaTravelTimes * (ptrdiff_t)sizeof(
			unsigned short
		));
}
	...
	while (updateListStart)
	{
		...
		for (i = 0, reach = file->GetArea(curUpdate->areaNum).rev_reach; reach; reach = reach->rev_next, i++)
		{
			...
			if (!areaCache->travelTimes[clusterAreaNum] || t < areaCache->travelTimes[clusterAreaNum])
			{
				...
				// reversed reachability used to get into this area
...
============
*/
bool idAASLocal::RouteToGoalArea(int areaNum, const idVec3 origin, int goalAreaNum, int travelFlags, int& travelTime,
...
============
*/
bool idAASLocal::FindNearestGoal(aasGoal_t& goal, int areaNum, const idVec3 origin, const idVec3& target,
                                 int travelFlags, aasObstacle_t* obstacles, int numObstacles,
