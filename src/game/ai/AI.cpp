...
void idAI::Think(void)
{
	...
	/*	this still draws in retail builds.. not sure why.. don't care at this point.
		if ( !aas && developer.GetBool() && !fl.hidden && !num_cinematics ) {
			gameRenderWorld->DrawText( "No AAS", physicsObj.GetAbsBounds().GetCenter(), 0.1f, colorWhite, gameLocal.GetLocalPlayer()->viewAngles.ToMat3(), 1, FPS_TO_MSEC() );
		}
	*/
...
float idAI::TravelDistance(const idVec3 &start, const idVec3 &end) const
{
	...
	if (!aas)
	{
		...
		{
			gameRenderWorld->DebugLine(colorBlue, start, end, FPS_TO_MSEC(), false);
...
bool idAI::GetMovePos(idVec3 &seekPos)
{
	...
	if (!result)
	{
		...
		if (ai_debugMove.GetBool())
		{
			gameRenderWorld->DebugLine(colorYellow, org, seekPos, FPS_TO_MSEC(), true);
		}
	}
	else
	{
...
void idAI::Turn(void)
{
	...
	else
	{
		...
		turnVel += AI_TURN_SCALE * diff * MS2SEC(FPS_TO_MSEC());
		if (turnVel > turnRate)
		{
			turnVel = turnRate;
		}
		...
		turnAmount = turnVel * MS2SEC(FPS_TO_MSEC());
		if ((diff >= 0.0f) && (turnAmount >= diff))
		{
			turnVel = diff / MS2SEC(FPS_TO_MSEC());
			turnAmount = diff;
		}
	...
	{
		const idVec3 &org = physicsObj.GetOrigin();
		gameRenderWorld->DebugLine(colorRed, org, org + idAngles(0, ideal_yaw, 0).ToForward() * 64, FPS_TO_MSEC());
		gameRenderWorld->DebugLine(colorGreen, org, org + idAngles(0, current_yaw, 0).ToForward() * 48, FPS_TO_MSEC());
		gameRenderWorld->DebugLine(colorYellow, org, org + idAngles(0, current_yaw + turnVel, 0).ToForward() * 32, FPS_TO_MSEC());
	}
}
...
void idAI::GetMoveDelta(const idMat3 &oldaxis, const idMat3 &axis, idVec3 &delta)
{
	...
	animator.GetDelta(gameLocal.time - FPS_TO_MSEC(), gameLocal.time, delta);
	delta = axis * delta;
...
void idAI::CheckObstacleAvoidance(const idVec3 &goalPos, idVec3 &newPos)
{
	...
	{
		gameRenderWorld->DebugLine(colorBlue, goalPos + idVec3(1.0f, 1.0f, 0.0f), goalPos + idVec3(1.0f, 1.0f, 64.0f), FPS_TO_MSEC());
		gameRenderWorld->DebugLine(foundPath ? colorYellow : colorRed, path.seekPos, path.seekPos + idVec3(0.0f, 0.0f, 64.0f), FPS_TO_MSEC());
	}
...
void idAI::AnimMove(void)
{
	...
	if (ai_debugMove.GetBool())
	{
		gameRenderWorld->DebugBounds(colorMagenta, physicsObj.GetBounds(), org, FPS_TO_MSEC());
		gameRenderWorld->DebugBounds(colorMagenta, physicsObj.GetBounds(), move.moveDest, FPS_TO_MSEC());
		gameRenderWorld->DebugLine(colorYellow, org + EyeOffset(), org + EyeOffset() + viewAxis[0] * physicsObj.GetGravityAxis() * 16.0f, FPS_TO_MSEC(), true);
		DrawRoute();
	}
}
...
idVec3 Seek(idVec3 &vel, const idVec3 &org, const idVec3 &goal, float prediction)
{
	...
	seekVel = goalDelta * MS2SEC(FPS_TO_MSEC());
...
void idAI::SlideMove(void)
{
	...
	vel -= vel * AI_FLY_DAMPENING * MS2SEC(FPS_TO_MSEC());
	vel += goalDelta * MS2SEC(FPS_TO_MSEC());
...
void idAI::AddFlyBob(idVec3 &vel)
{
	...
	if (fly_bob_strength)
	{
		...
		vel += fly_bob_add * MS2SEC(FPS_TO_MSEC());
		if (ai_debugMove.GetBool())
		{
			const idVec3 &origin = physicsObj.GetOrigin();
...
void idAI::AdjustFlyHeight(idVec3 &vel, const idVec3 &goalPos)
{
	...
	if (origin.z > goalPos.z)
	{
		...
		if (ai_debugMove.GetBool())
		{
			gameRenderWorld->DebugBounds(goLower ? colorRed : colorGreen, physicsObj.GetBounds(), path.endPos, FPS_TO_MSEC());
		}
	}
...
void idAI::AdjustFlySpeed(idVec3 &vel)
{
	...
	speed = vel.Normalize();
	speed += (move.speed - speed) * MS2SEC(FPS_TO_MSEC());
	if (speed < 0.0f)
	{
		speed = 0.0f;
	}
...
void idAI::FlyMove(void)
{
	...
	if (ai_debugMove.GetBool())
	{
		...
		gameRenderWorld->DebugBounds(colorOrange, physicsObj.GetBounds(), org, FPS_TO_MSEC());
		gameRenderWorld->DebugBounds(colorMagenta, physicsObj.GetBounds(), move.moveDest, FPS_TO_MSEC());
		gameRenderWorld->DebugLine(colorRed, org, org + physicsObj.GetLinearVelocity(), FPS_TO_MSEC(), true);
		gameRenderWorld->DebugLine(colorBlue, org, goalPos, FPS_TO_MSEC(), true);
		gameRenderWorld->DebugLine(colorYellow, org + EyeOffset(), org + EyeOffset() + viewAxis[0] * physicsObj.GetGravityAxis() * 16.0f, FPS_TO_MSEC(), true);
		DrawRoute();
	}
}
...
void idAI::StaticMove(void)
{
	...
	if (ai_debugMove.GetBool())
	{
		const idVec3 &org = physicsObj.GetOrigin();
		gameRenderWorld->DebugBounds(colorMagenta, physicsObj.GetBounds(), org, FPS_TO_MSEC());
		gameRenderWorld->DebugLine(colorBlue, org, move.moveDest, FPS_TO_MSEC(), true);
		gameRenderWorld->DebugLine(colorYellow, org + EyeOffset(), org + EyeOffset() + viewAxis[0] * physicsObj.GetGravityAxis() * 16.0f, FPS_TO_MSEC(), true);
	}
}
...
void idAI::UpdateEnemyPosition(void)
{
	...
	if (ai_debugMove.GetBool())
	{
		gameRenderWorld->DebugBounds(colorLtGrey, enemyEnt->GetPhysics()->GetBounds(), lastReachableEnemyPos, FPS_TO_MSEC());
		gameRenderWorld->DebugBounds(colorWhite, enemyEnt->GetPhysics()->GetBounds(), lastVisibleReachableEnemyPos, FPS_TO_MSEC());
	}
}
...
bool idAI::TestMelee(void) const
{
	...
	if (ai_debugMove.GetBool())
	{
		gameRenderWorld->DebugBounds(colorYellow, bounds, vec3_zero, FPS_TO_MSEC());
	}
...
bool idAI::UpdateAnimationControllers(void)
{
	...
	if (focusJoint != INVALID_JOINT)
	{
		...
		if (ai_debugMove.GetBool())
		{
			gameRenderWorld->DebugLine(colorRed, eyepos, eyepos + orientationJointAxis[0] * 32.0f, FPS_TO_MSEC());
		}
	}
	else
	{
...
#if 0
	gameRenderWorld->DebugLine( colorRed, orientationJointPos, focusPos, FPS_TO_MSEC() );
	gameRenderWorld->DebugLine( colorYellow, orientationJointPos, orientationJointPos + orientationJointAxis[ 0 ] * 32.0f, FPS_TO_MSEC() );
	gameRenderWorld->DebugLine( colorGreen, orientationJointPos, orientationJointPos + newLookAng.ToForward() * 48.0f, FPS_TO_MSEC() );
#endif
...
void idCombatNode::DrawDebugInfo(void)
{
	...
	for (ent = gameLocal.spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next())
	{
		...
		if (idMath::Fabs(cone_dot) > 0.1)
		{
			...
			gameRenderWorld->DebugLine(color, node->GetPhysics()->GetOrigin(), (pos1 + pos3) * 0.5f, FPS_TO_MSEC());
			gameRenderWorld->DebugLine(color, pos1, pos2, FPS_TO_MSEC());
			gameRenderWorld->DebugLine(color, pos1, pos3, FPS_TO_MSEC());
			gameRenderWorld->DebugLine(color, pos3, pos4, FPS_TO_MSEC());
			gameRenderWorld->DebugLine(color, pos2, pos4, FPS_TO_MSEC());
			gameRenderWorld->DebugBounds(color, bounds, org, FPS_TO_MSEC());
		}
	}
}
