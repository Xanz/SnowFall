...
===============
*/
void idPVS::AddPassageBoundaries(const idWinding& source, const idWinding& pass, bool flipClip, idPlane* bounds,
                                 int& numBounds, int maxBounds) const
{
	int i, j, k, l;
	idVec3 v1, v2, normal;
	float d, dist;
...
void idPVS::DrawPVS(const idVec3& source, const pvsType_t type) const
{
	...
	for (j = 0; j < numAreas; j++)
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			{
				gameRenderWorld->DebugLine(*color, (*portal.w)[k].ToVec3() + offset,
