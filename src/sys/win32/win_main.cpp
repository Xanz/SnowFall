...
idCVar Win32Vars_t::win_timerUpdate("win_timerUpdate", "0", CVAR_SYSTEM | CVAR_BOOL,
                                    "allows the game to be updated while dragging the window");
idCVar Win32Vars_t::win_allowMultipleInstances("win_allowMultipleInstances", "0", CVAR_SYSTEM | CVAR_BOOL,
...
const char* Sys_DefaultSavePath()
{
	...
	if (hShell)
	{
		SHGetKnownFolderPath_t SHGetKnownFolderPath = (SHGetKnownFolderPath_t)GetProcAddress(
			hShell, "SHGetKnownFolderPath");
		if (SHGetKnownFolderPath)
		{
			wchar_t* path;
			if (SUCCEEDED(
				SHGetKnownFolderPath( FOLDERID_SavedGames_IdTech5, CSIDL_FLAG_CREATE | CSIDL_FLAG_PER_USER_INIT, 0, &
					path )))
			{
              ...
              unsigned int& exitCode)
{
	...
	char* cmdLine = NULL;
	{
		...
		if (args != NULL)
		{
			if (appPath != NULL)
			{
				int len = idStr::Length(args) + idStr::Length(appPath) + 1 /* for space */ + 1 /* for NULL terminator */
					+ 2 /* app quotes */;
	...
	{
		// if waitMS == -1, don't wait for process to exit
		DWORD ec = 0;
		DWORD wait = 0;
		char buffer[4096];
		for (; ;)
...
const char* GetExceptionCodeInfo(UINT code)
{
	...
	case EXCEPTION_IN_PAGE_ERROR: return
	...
	case EXCEPTION_SINGLE_STEP: return
