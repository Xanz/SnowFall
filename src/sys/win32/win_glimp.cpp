...
void GLimp_TestSwapBuffers(const idCmdArgs& args)
{
	...
	{
		glSwapIntervalEXT(swapInterval);
...
void GLW_CheckWGLExtensions(HDC hDC)
{
	...
	wglGetPixelFormatAttribivARB = (PFNWGLGETPIXELFORMATATTRIBIVARBPROC)GLimp_ExtensionPointer(
		"wglGetPixelFormatAttribivARB");
	wglGetPixelFormatAttribfvARB = (PFNWGLGETPIXELFORMATATTRIBFVARBPROC)GLimp_ExtensionPointer(
...
static HGLRC CreateOpenGLContextOnDC(const HDC hdc, const bool debugContext)
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		const int glProfile = (useOpenGL32 == 1)
			                      ? WGL_CONTEXT_COMPATIBILITY_PROFILE_BIT_ARB
			                      : ((useOpenGL32 == 2) ? WGL_CONTEXT_CORE_PROFILE_BIT_ARB : 0);
		const int attribs[] =
		{
