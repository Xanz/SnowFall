...
void idLobby::HandlePacket(lobbyAddress_t& remoteAddress, idBitMsg fragMsg, idPacketProcessor::sessionId_t sessionID)
{
	...
	if (type == idPacketProcessor::RETURN_TYPE_OOB)
	{
		...
		else if (userData == OOB_MIGRATE_INVITE)
		{
			...
			ConnectTo(connectInfo, true);
			// Pass in true for the invite flag, so we can connect to invite only lobby if we need to
			...
			if (peerNum == host)
			{
				                                                  ...
				                                                  localSnap, fullSnap))
				{
					...
					if (sessionCB->GetState() != idSession::INGAME && sequence != -1)
					{
						...
						ackmsg.WriteQuantizedUFloat<
							idLobby::BANDWIDTH_REPORTING_MAX, idLobby::BANDWIDTH_REPORTING_BITS>(incomingBPS);
...
int idLobby::HandleInitialPeerConnection(idBitMsg& msg, const lobbyAddress_t& peerAddress, int peerNum)
{
	...
	if (MatchTypeIsJoinInProgress(parms.matchFlags))
	{
		...
		if ((lobbyType == TYPE_GAME || lobbyType == TYPE_GAME_STATE) && sessionCB->GetState() >= idSession::LOADING)
		{
			...
			{
				newPeer.pauseSnapshots = true;
				// Since this player joined in progress, let game dictate when to start sending snaps
			}
...
void idLobby::InitStateLobbyHost()
{
	...
	if (migrationInfo.state != MIGRATE_NONE)
	{
		migrationInfo.persistUntilGameEndsData.wasMigratedHost = true;
		// InitSessionUsersFromLocalUsers needs to know this
...
void idLobby::HandleReliableMsg(int p, idBitMsg& msg)
{
	...
	else if (reliableType == RELIABLE_USER_CONNECT_REQUEST)
	{
		...
		{
			// This assumes only one user in the msg
	...
	else if (reliableType == RELIABLE_CONNECT_AND_MOVE_TO_LOBBY)
	{
		...
		assert(destLobbyType > lobbyType);
		// Make sure this is a proper transition (i.e. TYPE_PARTY moves to TYPE_GAME, TYPE_GAME moves to TYPE_GAME_STATE)
...
void idLobby::BeginBandwidthTest()
{
	...
	for (int p = 0; p < peers.Num(); ++p)
	{
		...
		peers[p].bandwidthTestBytes = peers[p].packetProc->GetOutgoingBytes();
		// cache this off so we can see the difference when we are done
	}
}
...
void idLobby::ServerUpdateBandwidthTest()
{
	...
	for (int i = 0; i < peers.Num(); ++i)
	{
		...
		byte buffer[idPacketProcessor::MAX_OOB_MSG_SIZE];
		// <---- NOTE - When calling ProcessOutgoingMsg with true for oob, we can't go over this size
		idBitMsg msg(buffer, sizeof(buffer));
			...
			bandwidthSequenceNum >= net_bw_test_numPackets.GetInteger())
		{
			...
			peers[i].bandwidthTestBytes = sentBytes;
			// this now means total bytes sent (we don't care about starting/ending total bytes sent to peer)
...
bool idLobby::SendAnotherFragment(int p)
{
	...
	if (!peer.IsConnected())
	{
		// Not connected to any mode (party or game), so no need to send
		return false;
	}
