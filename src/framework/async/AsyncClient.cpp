...
void idAsyncClient::RunFrame(void)
{
	...
	do
	{
		...
		do
		{
			...
			newPacket = clientPort.GetPacketBlocking(from, msgBuf, size, sizeof(msgBuf), FPS_TO_MSEC() - (gameTimeResidual + clientPredictTime) - 1);
			if (newPacket)
			{
				msg.Init(msgBuf, sizeof(msgBuf));
				msg.SetSize(size);
	...
	} while (gameTimeResidual + clientPredictTime < FPS_TO_MSEC());
	...
	{
		usercmdGen->GetDirectUsercmd();
		gameTimeResidual = FPS_TO_MSEC() - 1;
		clientPredictTime = 0;
		return;
	}
	...
	if (gameTimeResidual + clientPredictTime >= FPS_TO_MSEC())
	{
		lastFrameDelta = 0;
	}
	...
	while (gameTimeResidual + clientPredictTime >= FPS_TO_MSEC())
	{
		...
		// update time
		gameFrame++;
		gameTime += FPS_TO_MSEC();
		gameTimeResidual -= FPS_TO_MSEC();
		...
		while (snapshotGameFrame < gameFrame)
		{
			...
			bool lastPredictFrame = (snapshotGameFrame + 1 >= gameFrame && gameTimeResidual + clientPredictTime < FPS_TO_MSEC());
