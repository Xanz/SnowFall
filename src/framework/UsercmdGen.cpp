...
void idUsercmdGenLocal::AdjustAngles(void)
{
	...
	{
		speed = idMath::M_MS2SEC * FPS_TO_MSEC() * in_angleSpeedKey.GetFloat();
	}
	else
	{
		speed = idMath::M_MS2SEC * FPS_TO_MSEC();
	}
...
void idUsercmdGenLocal::JoystickMove(void)
{
	...
	if (toggled_run.on ^ (in_alwaysRun.GetBool() && idAsyncNetwork::IsActive()))
	{
		anglespeed = idMath::M_MS2SEC * FPS_TO_MSEC() * in_angleSpeedKey.GetFloat();
	}
	else
	{
		anglespeed = idMath::M_MS2SEC * FPS_TO_MSEC();
	}
