...
bool idDict::GetMatrix(const char* key, const char* defaultString, idMat3& out) const
{
	...
	sscanf(s, "%f %f %f %f %f %f %f %f %f", &out[0].x, &out[0].y, &out[0].z, &out[1].x, &out[1].y, &out[1].z, &out[2].x,
	       &out[2].y, &out[2].z);
	return found;
}
...
void idDict::WriteToFileHandle(idFile* f) const
{
	...
	{
		// don't loop on the swapped count use the original
...
void idDict::WriteToIniFile(idFile* f) const
{
	...
	idTempArray<int> prefixIndex(sortedArgs.Num());
	// for each keyValue in the args, this is an index into which prefix it uses.
...
bool idDict::ReadFromIniFile(idFile* f)
{
	...
	// Since the .ini files are not null terminated, make sure we mark where the end of the .ini file is in our read buffer
